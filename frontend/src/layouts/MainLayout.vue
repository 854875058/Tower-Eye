...
const menuItems = [
  ...
  { key: 'workbench', title: '数据工作台', icon: 'Files', path: '/workbench' },
  { key: 'tower-demo', title: '视联样机', icon: 'MagicStick', path: '/tower-demo' },
...
const currentTitle = computed(() => {
  return menuItems.find(item => currentPath.value.startsWith(item.path))?.title || '智能问答平台'
})
const isFullWidthPage = computed(() => ['/query', '/config', '/workbench'].includes(route.path))
...
const handleWorkspaceChange = (workspace: any) => {
  userStore.setCurrentWorkspace(workspace)
}
...
const checkLlmHeartbeat = async () => {
  try {
    ...
    llmMonitor.value = {
      status: res.data.ok ? 'up' : 'down',
      message: res.data.message || (res.data.ok ? '环境启动正常' : '环境启动异常'),
    }
  } catch (_error) {
    llmMonitor.value = {
      status: 'down',
...
<template>
  <div class="app-container">
    <aside class="app-sidebar">
      ...
      <el-menu :default-active="activeMenu" class="sidebar-menu" :router="false">
        <el-menu-item
    ...
    <main class="app-main">
      ...
      <section class="main-content" :class="{ 'no-padding': isFullWidthPage }">
        <router-view />
      </section>
    </main>
  </div>
