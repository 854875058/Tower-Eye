// 用户相关
export interface User {
  id: number
  username: string
  email?: string
  full_name?: string
  is_active: boolean
  created_at: string
}

export interface LoginParams {
  username: string
  password: string
}

export interface LoginResponse {
  access_token: string
  token_type: string
}

// 工作空间
export interface Workspace {
  id: number
  name: string
  description?: string
  created_at: string
}

// 数据源
export interface DataSource {
  id: number
  workspace_id: number
  name: string
  type: 'mysql' | 'postgresql' | 'sqlserver' | 'csv' | 'duckdb'
  host?: string
  port?: number
  database?: string
  username?: string
  is_active: boolean
  created_at: string
  updated_at: string
}

export interface DataSourceCreate {
  name: string
  type: string
  host?: string
  port?: number
  database?: string
  username?: string
  password?: string
  connection_string?: string
  workspace_id: number
}

export interface DataSourceTestRequest {
  type: string
  host?: string
  port?: number
  database?: string
  username?: string
  password?: string
  connection_string?: string
}

export interface SchemaColumn {
  table_name: string
  column_name: string
  column_type?: string
  description?: string
  is_primary_key: boolean
  is_foreign_key: boolean
  is_nullable: boolean
  sample_values?: any[]
  row_count_estimate?: number
}

// 数据集
export interface Dataset {
  id: number
  workspace_id: number
  data_source_id?: number
  data_source_ids?: number[]
  name: string
  description?: string
  status: 'draft' | 'active' | 'deprecated'
  processing_status: 'pending' | 'processing' | 'ready' | 'failed'
  progress?: number
  error_message?: string
  media_count: number
  processed_count: number
  failed_count: number
  last_processed_at?: string
  metrics?: MetricDefinition[]
  dimensions?: DimensionDefinition[]
  aliases?: AliasMapping[]
  business_rules?: string
  version: number
  created_at: string
}

export interface MetricDefinition {
  name: string
  expression: string
  description?: string
  aggregation_type: string
}

export interface DimensionDefinition {
  name: string
  column: string
  description?: string
}

export interface AliasMapping {
  alias: string
  column: string
  description?: string
}

export interface DatasetCreate {
  workspace_id?: number
  name: string
  description?: string
  data_source_ids?: number[]
  data_source_id?: number
  file_paths?: string[]
  metrics?: MetricDefinition[]
  dimensions?: DimensionDefinition[]
  aliases?: AliasMapping[]
  business_rules?: string
  status?: string
}

// 智能标注
export interface WorkbenchSftpConfig {
  host: string
  port: number
  username: string
  password: string
  remote_paths: string[]
  recursive: boolean
}

export interface WorkbenchDatasetCreate {
  workspace_id: number
  name: string
  description?: string
  file_paths?: string[]
  sftp_config?: WorkbenchSftpConfig
}

export interface WorkbenchSampleImportRequest {
  workspace_id: number
  name?: string
  description?: string
  max_records?: number
}

export interface WorkbenchResource {
  id: number
  dataset_id: number
  source_kind: 'upload' | 'path' | 'sftp'
  source_uri: string
  stored_path: string
  file_name: string
  file_extension?: string
  file_category?: string
  mime_type?: string
  file_size?: number
  checksum?: string
  parser_name?: string
  status: 'pending' | 'processing' | 'ready' | 'failed'
  chunk_count: number
  text_length: number
  extraction_metadata?: Record<string, any>
  labels?: string[]
  category_label?: string
  cluster_id?: number
  cluster_label?: string
  summary_text?: string
  quality_score?: number
  processing_metadata?: Record<string, any>
  error_message?: string
  created_at: string
  updated_at: string
  last_processed_at?: string
}

export interface WorkbenchDataset {
  id: number
  workspace_id: number
  name: string
  description?: string
  processing_status: 'pending' | 'processing' | 'ready' | 'failed'
  resource_count: number
  processed_count: number
  failed_count: number
  chunk_count: number
  vector_count: number
  total_text_length: number
  source_summary?: Record<string, any>
  processing_summary?: Record<string, any>
  error_message?: string
  last_ingested_at?: string
  processing_updated_at?: string
  created_at: string
  updated_at: string
}

export interface WorkbenchDatasetDetail extends WorkbenchDataset {
  resources: WorkbenchResource[]
}

export interface WorkbenchSearchRequest {
  query: string
  top_k?: number
}

export interface WorkbenchSearchResult {
  dataset_id: number
  resource_id: number
  chunk_id: number
  chunk_index: number
  file_name: string
  source_uri: string
  score: number
  content: string
  keywords: string[]
  chunk_metadata?: Record<string, any>
  parser_name?: string
}

export interface WorkbenchProcessingRequest {
  extract_labels: boolean
  cluster_count: number
  refresh_summary: boolean
}

export interface TowerDemoRecommendation {
  type: string
  title: string
  reason: string
  priority: string
  risk_score?: number
  action?: string
}

export interface TowerDemoEvidenceEvent {
  event_id: string
  event_type: string
  county_name?: string
  city_name?: string
  device_name?: string
  alarm_time?: string
  confidence_level?: number
  summary?: string
  image_url?: string
  video_url?: string
  risk_score?: number
  priority?: string
  action_type?: string
}

export interface TowerDemoDashboard {
  summary: {
    total_events: number
    distinct_devices: number
    distinct_counties: number
    distinct_cities: number
    avg_confidence: number
    min_confidence: number
    max_confidence: number
    time_range?: { start?: string; end?: string }
    image_count: number
    video_count: number
  }
  hot_regions: Array<Record<string, any>>
  hot_devices: Array<Record<string, any>>
  low_confidence_events: TowerDemoEvidenceEvent[]
  trend: Array<{ day: string; count: number }>
  priority_queue: TowerDemoEvidenceEvent[]
  recommendations: TowerDemoRecommendation[]
}

export interface TowerDemoAssistantResponse {
  question: string
  answer: string
  steps: Array<{ step: string; detail: string }>
  actions?: Array<{
    type: string
    label: string
    target: string
    payload?: Record<string, any>
  }>
  visualization?: {
    chart_type: string
    title: string
    categories: string[]
    series: Array<{ name: string; data: Array<number> }>
    y_axis_name?: string
  } | null
  evidence: any
  dashboard?: TowerDemoDashboard | null
}

export interface TowerGraphOntologyItem {
  type: string
  label: string
  description: string
  key_fields: string[]
}

export interface TowerGraphNode {
  id: string
  label: string
  category: string
  symbolSize: number
  value?: number
  meta?: Record<string, any>
}

export interface TowerGraphEdge {
  source: string
  target: string
  relation: string
  label: string
}

export interface TowerGraphPayload {
  ontology: TowerGraphOntologyItem[]
  graph: {
    nodes: TowerGraphNode[]
    edges: TowerGraphEdge[]
    stats: {
      node_count: number
      edge_count: number
      node_categories: Record<string, number>
      edge_categories: Record<string, number>
    }
  }
}

export interface TowerLineageStep {
  stage: string
  title: string
  description: string
  evidence: Record<string, any>
}

export interface TowerLineagePayload {
  event: Record<string, any>
  timeline: TowerLineageStep[]
  recommendation: Record<string, any>
}

export interface TowerOntologyEntityConfig {
  type: string
  label: string
  description: string
  key_fields: string[]
  source_fields: string[]
  enabled: boolean
}

export interface TowerOntologyRelationConfig {
  type: string
  label: string
  source_type: string
  target_type: string
  description: string
  source_field?: string
  enabled: boolean
}

export interface TowerOntologyConfig {
  entities: TowerOntologyEntityConfig[]
  relations: TowerOntologyRelationConfig[]
  updated_at?: string
}

export interface WorkbenchSubscriptionCreate {
  workspace_id: number
  dataset_id: number
  name: string
  source_kind: 'path' | 'sftp'
  interval_minutes: number
  is_enabled: boolean
  local_paths?: string[]
  sftp_config?: WorkbenchSftpConfig
}

export interface WorkbenchSubscriptionUpdate {
  name?: string
  interval_minutes?: number
  is_enabled?: boolean
  local_paths?: string[]
  sftp_config?: WorkbenchSftpConfig
}

export interface WorkbenchSubscription {
  id: number
  workspace_id: number
  dataset_id: number
  name: string
  source_kind: 'path' | 'sftp'
  is_enabled: boolean
  interval_minutes: number
  local_paths: string[]
  source_config?: Record<string, any>
  last_status: 'idle' | 'running' | 'success' | 'error'
  last_message?: string
  last_run_at?: string
  next_run_at?: string
  last_new_resources: number
  created_at: string
  updated_at: string
}

export type AnnotationMediaType = 'image' | 'video'

export interface AnnotationClassInfo {
  key: string
  class_name: string
  label: string
  class_id: number
}

export interface AnnotationEngineInfo {
  vision_available: boolean
  yolo_available: boolean
  vision_model?: string | null
  yolo_model_path?: string | null
  confidence_threshold?: number
}

export interface AnnotationMeta {
  classes: AnnotationClassInfo[]
  engine: AnnotationEngineInfo
  defaults: {
    image_dir?: string
    video_dir?: string
    output_dir?: string
  }
}

export interface AnnotationBox {
  class: string
  class_id: number
  confidence: number
  bbox: number[]
  manual: boolean
  track_id?: number
  age?: number
  hits?: number
}

export interface AnnotationItem {
  id: string
  order_index: number
  file_name: string
  status: 'pending' | 'processing' | 'ready' | 'failed'
  error_message?: string | null
  description?: string
  annotations: AnnotationBox[]
  stats: Record<string, any>
  width?: number | null
  height?: number | null
}

export interface AnnotationSession {
  id: string
  workspace_id: number
  media_type: AnnotationMediaType
  source_dir: string
  output_dir: string
  status: 'pending' | 'processing' | 'ready' | 'failed'
  current_index: number
  progress: {
    total: number
    processed: number
    failed: number
    percent: number
    message: string
  }
  options: {
    use_tracking: boolean
    frame_interval: number
    detect_size: number
  }
  engine: AnnotationEngineInfo
  items: AnnotationItem[]
  restored?: boolean
}

export interface AnnotationSessionRequest {
  workspace_id: number
  media_type: AnnotationMediaType
  source_dir: string
  output_dir?: string
  use_tracking?: boolean
  frame_interval?: number
  detect_size?: number
  force_reprocess?: boolean
}

export interface AnnotationOperationResponse {
  message: string
  saved_paths?: string[]
}

// 查询
export interface QueryRequest {
  question: string
  workspace_id: number
  dataset_id?: number
  table_names?: string[]  // 用户选择的表名列表
  context?: QuerySessionContext
  query_image_path?: string
  query_video_path?: string
}

export interface QueryContextTurn {
  question: string
  intent?: string
  answer?: string
  sql?: string
  row_count?: number
  result_schema?: { name: string; type: string }[]
  table_names?: string[]
  dataset_id?: number
  status?: string
  plan_source?: string
}

export interface QuerySessionContext {
  recent_turns?: QueryContextTurn[]
  current_dataset_id?: number
  current_table_names?: string[]
}

export interface QueryResponse {
  question: string
  normalized_question?: string
  intent?: string
  intent_text?: string
  sql?: string
  sql_params?: any[]
  matched_dataset?: { id: number; name: string }
  semantic_sql?: string
  executable_sql?: string
  reasoning_summary?: string
  result_schema?: { name: string; type: string }[]
  result_rows?: Record<string, any>[]
  row_count: number
  chart_suggestion?: string
  cost_time_ms?: number
  cost_rows?: number
  warnings?: string[]
  status: string
  error?: string
  trace_id: string
  audit_id: string
  agent_steps?: AgentStep[]
  execution_history?: Record<string, any>[]
  evidence?: Record<string, any>
  semantic_scores?: Record<string, number>
  vector_only_results?: Record<string, any>[]
  answer?: string
  plan_source?: string
  confidence?: number
  clarification_needed?: boolean
  clarification_options?: string[]
  executing?: boolean  // 用于标记 SQL 是否正在重新执行
  // 保存生成 SQL 时使用的表名，用于重新执行
  table_names?: string[]
  dataset_id?: number
}

export interface AgentStep {
  step: number
  type: 'thought' | 'action' | 'observation'
  content?: string
  action?: string
  input?: string
}

// 历史记录
export interface QueryHistory {
  id: number
  question: string
  normalized_question?: string
  intent?: string
  semantic_sql?: string
  executable_sql?: string
  row_count: number
  execution_time_ms?: number
  status: string
  trace_id: string
  created_at: string
}

// 系统监控
export interface LlmHeartbeatResponse {
  ok: boolean
  status: 'up' | 'down'
  message: string
  checked_at: string
  latency_ms?: number
  model?: string | null
  model_count?: number
  detail?: string
}
