...
from app.services.tower_demo import WARNING_IMG_ROOT, WARNING_VIDEO_ROOT
from app.services.trace import init_trace_manager
from app.services.workbench import ensure_workbench_dirs
from app.services.workbench_subscription_tasks import get_workbench_subscription_manager
...
if WARNING_IMG_ROOT.exists():
    app.mount("/tower-warning-img", StaticFiles(directory=str(WARNING_IMG_ROOT)), name="tower-warning-img")
if WARNING_VIDEO_ROOT.exists():
    app.mount("/tower-warning-video", StaticFiles(directory=str(WARNING_VIDEO_ROOT)), name="tower-warning-video")
...
async def setup_trace_system():
    ...
    await get_workbench_task_manager().startup()
    await get_workbench_subscription_manager().startup()
...
async def shutdown_background_workers():
    ...
    await get_workbench_task_manager().shutdown()
    await get_workbench_subscription_manager().shutdown()
