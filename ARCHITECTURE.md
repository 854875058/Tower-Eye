...
│                    Agent 层 (LangGraph)                          │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  状态机 (State Machine) — 双路融合架构                    │  │
│  │  ┌──────┐  ┌───────┐  ┌──────┐  ┌──────┐  ┌────────┐   │  │
│  │  │Parse │→│SQL Cache│→│Validate│→│Execute│→│Semantic│   │  │
│  │  │      │  │ 命中?  │  │  SQL  │  │  SQL │  │Enhance │   │  │
│  │  │      │  │ ↓ miss │  └───┬───┘  └───┬──┘  └───┬────┘   │  │
│  │  └──┬───┘  │ LLM生成│    ↓ error   ↓ error     ↓        │  │
│  │     │      └───────┘  ┌──────┐    ┌──────┐              │  │
│  │     │ search          │ Fix  │←───┘│Format│→ END        │  │
│  │     ↓ intent          │ SQL  │     │Answer│              │  │
│  │  ┌──────────┐         └──────┘     └──────┘              │  │
...
│  │  └──────────┘                                            │  │
│  │                                                          │  │
│  │  Semantic Enhance (双路融合):                             │  │
│  │  SQL 结果 + 向量检索 → 语义匹配分数 + 推荐补充          │  │
│  └──────────────────────────────────────────────────────────┘  │
└────────────────────────┬────────────────────────────────────────┘
                         │
┌────────────────────────▼────────────────────────────────────────┐
...
- ✅ **双路融合**：list 查询在 SQL 执行后自动做语义增强（向量检索匹配 + 推荐补充）
...
START → 解析问题 → [search intent] → 向量检索 → 格式化答案 → END
              ↓ [list/count]
         SQL缓存查找 → [命中] → 验证SQL → 执行SQL → [list] → 语义增强 → 格式化答案 → END
                        ↓ [未命中]                    [count] → 格式化答案 → END
                    LLM 生成SQL → 验证SQL → 执行SQL → ...
                                   ↓ 失败      ↓ 失败
                                 修复SQL ←──────┘
                                   ↓ 重试
                                 验证SQL

语义增强 (Semantic Enhance):
  SQL 结果 ──┐
             ├→ file_path 匹配 → 附加语义分数 → 重排序
  向量检索 ──┘
             └→ 独有结果 → "您可能还感兴趣" 推荐
```
...
| `list` | 查询/查看/列出 + 结构化条件 | NL2SQL → SQL 执行 → **语义增强** |
...
### 5.1.1 **双路融合 — 语义增强** (`poc/qa/agent.py: semantic_enhance_node`)

**功能**：list 查询在 SQL 执行成功后，自动做一次向量检索，将结构化查询结果与图像语义检索结果融合。

**工作原理**：
1. SQL 查出 N 条结果（保底，确保结构化条件精确匹配）
2. 同时用用户问题文本编码向量，在 LanceDB 中做 hybrid_search
3. 通过 `file_path` 文件名匹配两路结果：
   - 交集部分：附加语义相似度分数（`hybrid_score`），展示在详情标题上
   - 向量检索独有结果：作为"您可能还感兴趣"推荐（最多 5 条）
4. 每条记录的 VL 图像理解摘要（`summary`）在详情页展示

**展示效果**：
- 列表标题显示 `语义匹配 M/N` 徽章
- 每条详情标题显示 `[语义 85%]` 分数
- 详情页顶部展示 VL 生成的图像理解摘要（紫色背景）
- 底部"您可能还感兴趣"区域展示向量检索独有结果（含缩略图 + 语义分数）

**容错设计**：
- LanceDB 不可用或向量检索失败时，静默跳过，不影响 SQL 主流程
- 语义增强是增值功能，不是必要路径
